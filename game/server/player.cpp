...
// wait for any button down,  or mp_forcerespawn is set and the respawn time is up, or if the player is a bot respawn automatically
	if (!fAnyButtonDown 
		&& !( g_pGameRules->IsMultiplayer() && forcerespawn.GetInt() > 0 && (gpGlobals->curtime > (m_flDeathTime + 5)) ) && !this->IsBot() )
		return;
